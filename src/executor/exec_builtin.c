...
/*   Updated: 2025/07/06 18:06:53 by plichota         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int execute_builtin(t_ast *ast, int fd_out, t_sh *shell)
{
	...
	if (ft_strcmp(cmd, "echo") == 0)
		return (execute_builtin_echo(ast, fd_out));
	if (ft_strcmp(cmd, "cd") == 0)
		return execute_builtin_cd(ast, shell);
	if (ft_strcmp(cmd, "pwd") == 0)
		return execute_builtin_pwd(ast, fd_out);
	if (ft_strcmp(cmd, "export") == 0)
		return execute_builtin_export(ast, fd_out, shell);
	if (ft_strcmp(cmd, "unset") == 0)
		return execute_builtin_unset(ast, shell);
	if (ft_strcmp(cmd, "env") == 0)
		return execute_builtin_env(ast, fd_out, shell);
